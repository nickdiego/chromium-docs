...
- OTOH, for WebSockets, [network::WebSocket](https://source.chromium.org/chromium/chromium/src/+/main:services/network/websocket.h;l=54;drc=861e5739e4a59f36e209efef954193030ca234a9) is the "equivalent" to URLLoader, as it also encapsulates communication with NetworkService and browser process to deal with auth challenges, etc. It also indirectly owns an URLRequest (through WebSocketChannel => WebSocketStream => WebSocketStreamRequest, as can be seen in the class diagram above). However, as of now, it does not handle client certificate requests coming through URLLoaderNetworkServiceObserver. **That's that gap this proposal aims to address.**
